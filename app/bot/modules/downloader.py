import traceback
import asyncio, re, os, logging, glob, orjson, ijson, signal
...
class Downloader(object):
    ...
    async def send_results(self) -> None:
        ...
        if self.result['cover']:
            if self.status != DOWNLOAD_STATUS.ERROR:
                if os.path.exists(self.result['cover']):
                    sf = True
                    await self.bot.messages_queue.add( 'send_photo', chat_id=self.task.chat_id, photo=self.result['cover'], request_timeout=600000 )
            else:
        ...
        if self.result['file']:
            ...
            if os.path.exists(self.result['file']):
                sf = True
                await self.bot.messages_queue.add( 'send_document', chat_id=self.task.chat_id, document=self.result['file'], caption=self.result['caption'], parse_mode='MarkdownV2', request_timeout=600000 )
                ...
                async for files in self.__chunked_media_group():
                    ...
                    for file in files:
                        if os.path.exists(file):
                            ...
                            media.append(m)
                    if media:
                        await self.bot.messages_queue.add( 'send_media_group', chat_id=self.task.chat_id, media=media, request_timeout=600000 )
            else:
                await self.bot.messages_queue.add( 'send_document', chat_id=self.task.chat_id, document=self.result['files'][0], caption=self.result['caption'], parse_mode='MarkdownV2', request_timeout=600000 )
        ...
        await self.__delete_sending_notice()
    ...
    async def __get_last_line(self) -> str:
        ...
        if not os.path.exists(self._log_file):
            return ''
        # async with aiofiles.open(self._log_file, mode='r') as f:
            # async for line in f:
                # pass
    ...
    async def __chunked_media_group(self) -> Iterator[Union[str, Path]]:
        for i in range(0, len(self.result['files']), 5):
            yield self.result['files'][i:i + 5]
    ...
    async def __download__prepare_args(self) -> None:
        ...
        if task.site in ['litmarket.ru','litres.ru','litnet.com','ficbook.net']:
            args.append('--delay')
            args.append('1')

        if task.site in ['ranobelib.me','mangalib.me']:
            args.append('--delay')
            args.append('3')
    ...
    async def __download__get_files(self) -> None:
        ...
        if _downloaders_path and self._args:
            ...
            # with open(self._log_file,'w') as log:
                # self._process = await asyncio.create_subprocess_shell(command, stdout=log, stderr=asyncio.subprocess.STDOUT, cwd=_downloaders_path, preexec_fn=os.setsid)
                # self._process = await asyncio.create_subprocess_exec(_exec, *command, stdout=log, stderr=asyncio.subprocess.STDOUT, cwd=_downloaders_path, preexec_fn=os.setsid)
            self._process = await asyncio.create_subprocess_shell(command, cwd=_downloaders_path, preexec_fn=os.setsid)
    ...
    async def __process_data__caption(self) -> None:
        ...
        try:
            if self._json is not None and _downloaders_path:
                command = ['./json_process', f'"{self._json}"']
                command = ' '.join(command)
                ...
                with open(self._json, "r") as f:
                    ...
                    _book_title    = _json['book_title']
                    _book_url      = _json['book_url']
                    _author_name   = _json['author_name']
                    _author_url    = _json['author_url']
                    _seria_name    = _json['seria_name']
                    _seria_number  = _json['seria_number']
                    _seria_url     = _json['seria_url']
                    _book_chapters = _json['chapters']
    ...
    async def __process_files__maybe_convert(self) -> None:
        ...
        if self.task.target_format and _converters_path:
            ...
            self._process = await asyncio.create_subprocess_exec(_exec, *command, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, cwd=_converters_path, preexec_fn=os.setsid)
            await self._process.wait()
    ...
    async def __process_files__maybe_split(self) -> None:
        ...
        if self.result['file']:
            ...
            if fsize > _split_limit:
                ...
                sfs = f'{sfs}m'
                cmd1 = f'cd {self._files_dir}; zip -s {sfs} "{splitted_file}" "{file}"'
                cmd2 = f'cd {self._files_dir}; rm -rf "{file}"'

                proc = await asyncio.create_subprocess_shell(cmd1)
                await proc.wait()

                proc = await asyncio.create_subprocess_shell(cmd2)
                await proc.wait()
    ...
    async def __delete_sending_notice(self) -> None:
        await asyncio.sleep(1)
        await self.bot.messages_queue.add( 'delete_message', chat_id=self.task.chat_id, message_id=self.task.message_id )