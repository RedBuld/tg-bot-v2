...
class DB(object):
	...
	async def init(self, _sync=False) -> None:
		try:
			self.engine = create_async_engine(self.bot.config.get('DB_URL'), pool_size=1, max_overflow=50, pool_recycle=60, pool_pre_ping=True)
	...
	async def is_blocked_link(self, link: str) -> Optional[str]:
		res = None

		async with self.engine.begin() as conn:
			query = await conn.execute(
				select(BlockedLinks)\
					.filter(BlockedLinks.link==link)
			)
			res = query.fetchone()
			if res:
				keys = query.keys()
				res = await self.__map_one__(res,keys,BlockedLinks)
				return res.reason
		return res
	...
	async def update_user_usage_extended(self, user_id: int, site: str) -> Optional[int]:
		...
		async with self.engine.begin() as conn:
			res = await conn.execute(insert(UserUsageExtended.__table__).values(**params).on_duplicate_key_update({
				"count":UserUsageExtended.count + 1,
	...
	async def update_user_usage(self, user_id: int) -> Optional[int]:
		...
		async with self.engine.begin() as conn:
			res = await conn.execute(insert(UserUsage.__table__).values(**params).on_duplicate_key_update({
	...
	async def update_bot_stat(self, queue_length: int, queue_act: int, queue_limit: int, queue_sim: int) -> bool:
		...
		async with self.engine.begin() as conn:
			res = await conn.execute(insert(BotStat.__table__).values(**params).on_duplicate_key_update({
				"queue_length":params['queue_length'],
				"queue_limit":params['queue_limit'],
				"queue_sim":params['queue_sim'],
				"queue_act":params['queue_act'],
				"last_on":params['last_on']
			}))
			await conn.commit()
		return res.lastrowid if res else None
	...
	async def add_site_stat(self, site: str, size: int) -> Optional[int]:
		...
		async with self.engine.begin() as conn:
			stmt = insert(SiteStat.__table__).values(**params).on_duplicate_key_update({
				"count":SiteStat.count + 1,
				"fsize":SiteStat.fsize + size
			})
