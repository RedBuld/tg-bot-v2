...
class __Config__(object):
	...
	__GLOBAL_STORE = {
		...
		'DOWNLOADS_SPLIT_LIMIT'       : 399*1024*1024,
	...
	def __load_env__(self) -> None:
		...
		if not self.__loaded__: # IMMUTABLE DURING EXECUTION
			self.__GLOBAL_STORE['DB_URL'] = os.getenv('DB_URL',None)
		...
		self.__GLOBAL_STORE['DOWNLOADERS_TEMP_PATH'] = os.path.join( os.getenv('DOWNLOADERS_TMP_PATH',self.__GLOBAL_STORE['DOWNLOADERS_PATH']), 'temp' )
		self.__GLOBAL_STORE['DOWNLOADERS_LOG_PATH'] = os.path.join( os.getenv('DOWNLOADERS_TMP_PATH',self.__GLOBAL_STORE['DOWNLOADERS_PATH']), 'logs' )
	...
	def __load_json__(self, store, file) -> None:
		with open(file,'r') as _config_file:
			...
			if 'accept' in _config:
				store['ACCEPT_NEW'] = bool(_config['accept'])
	...
	def __validate__(self):
		...
		if not self.get('DB_URL'):
			raise Exception('DB_URL/db_url not set')
		...
		if not os.path.exists( self.get('DOWNLOADERS_TEMP_PATH') ):
			os.makedirs( self.get('DOWNLOADERS_TEMP_PATH'), 0o777, exist_ok=True)

		if not os.path.exists( self.get('DOWNLOADERS_LOG_PATH') ):
			os.makedirs( self.get('DOWNLOADERS_LOG_PATH'), 0o777, exist_ok=True)
		...
		if self.get('LOCAL_SERVER'):
			__max_file_size = 399*1024*1024 # Telegram API limit for local server is 2GB
